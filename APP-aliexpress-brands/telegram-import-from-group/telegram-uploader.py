...
def createMsgTXT(count):
    ...
    if count != '0':
        ...
        txt = f'{new_title} \\|\t\t {price}\n\n ' \
              f'__Please Follow Instructions placed into product images__ ☝🏻 \n\n\t\t' + \
              f'\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t🫴   [Buy It Now]({link})'
...
def sendMediaGroup(images, caption='new message', reply_to_message_id=None):
    ...
    response = requests.post(mediaGroupURL, data={'chat_id': chat_id,
                                                  'media': json.dumps(media),
                                                  'reply_to_message_id': reply_to_message_id,
                                                  'disable_notification': 'true'
                                                  }, files=files, verify=False)
    logger.debug(f'resp: {response.text}')
...
def printStatusNotFound(i):
    msg = f'[ID:{i}] [FAILED] {errorRate}/{errorRate + successRate} {title} {price} {link}'
    # msg = f'[ID:{id}] [FAILED] {errorRate}/{errorRate + successRate} {title} {price} {link} ERROR -> status:{resp.status_code} {resp.text}'
...
def saveIdToCFG(id):
    ...
    cfg_file = open('config_files/uploader_config.ini', 'w')
    ...
    with open(csv_path, encoding='utf-8') as csv_file:
        for line in csv.DictReader(csv_file):
            if line is not None and line['id'] == msg_id and line['affiliate_link'][:3] == 'htt':
                ...
                if price != '$' and price != '' and price != 'failed':
                ...
                if link.__contains__('click.aliexpress'):
                    ...
                    if check_product_existence(link):
                        orderCount = get_product_details(link)
                        orig_price = get_orig_price(link)
                        msgTxt = createMsgTXT(orderCount)
                        resp = sendMediaGroup(images=images_path_list, caption=msgTxt)
                        logger.debug(f'sleeping {timeout} seconds')
                        code = resp.status_code
                    else:
                        logger.warning(f'[ID:{msg_id}] product doesn\'t exists CONTINUE TO NEXT ITEM')
                        printStatusNotFound(msg_id)
                        code, orderCount, orig_price = '', '', ''
